...
export default function History() {
  ...
  const handleDownload = async (video) => {
    ...
    // If blob is not in memory, try to fetch from storageUrl
    if ((!blob || !(blob instanceof Blob)) && video.storageUrl) {
      try {
        blob = await fetch(video.storageUrl).then((r) => r.blob());
      } catch (e) {
        console.warn("Failed to fetch blob from storageUrl:", e);
      }
    }

    // If we have a video blob, try to save/share it
    if (blob && blob instanceof Blob) {
      ...
      if (Capacitor.isNativePlatform()) {
        try {
          ...
          console.warn("Capacitor Filesystem failed, trying Share:", fsErr);
        }
      }
                ...
                {video.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {video.tags.slice(0, 4).map((tag, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded-full bg-dark-600 text-xs text-white/50"
                      >
                        #{tag}
                      </span>
                    ))}
