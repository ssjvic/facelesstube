...
async function callBackendAI(idea, language, template = 'general') {
    console.log('🖥️ Calling backend AI...', { template })
    const response = await fetch(`${API_BASE}/api/ai/generate-script`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idea, language, template })
    })
...
export const generateScript = async (idea, language = 'es', template = 'general') => {
    if (!idea || idea.trim().length < 3) {
        throw new Error('Escribe una idea para tu video')
    }

    try {
        return await callBackendAI(idea, language, template)
    } catch (error) {
