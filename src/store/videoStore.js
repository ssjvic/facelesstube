...
export const useVideoStore = create((set, get) => ({
  ...
  loadVideos: async (userId) => {
    ...
    if (isSupabaseConfigured()) {
      try {
        ...
        // Also try to restore blobs from IndexedDB for videos without storage_url
        try {
          ...
          if (idbVideos.length > 0) {
            ...
            mappedVideos = mappedVideos.map((v) => {
              if (!v.videoUrl) {
          ...
          console.warn("IndexedDB blob restore (Supabase path):", e);
        }
  ...
  completeGeneration: async (videoData, userId) => {
    const newVideo = {
      ...
      videoUrl:
        videoData.videoUrl ||
        (videoData.blob ? URL.createObjectURL(videoData.blob) : null),
    };
  ...
  deleteVideo: async (videoId, userId) => {
    if (isSupabaseConfigured() && userId) {
      ...
      localStorage.setItem(
        "facelesstube_videos",
        JSON.stringify(videos.map((v) => ({ ...v, blob: null }))),
      );
    }
